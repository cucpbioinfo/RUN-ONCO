...
import run.onco.batch.api.utils.AutowiringSpringBeanJobFactory;
...
public class SchedulerServiceImpl implements SchedulerService {
	...
	public void register(JobDetail jobDetail, CronTrigger cronTrigger) {
		...
		try {
			/*
			 * We have 3 ways to restart the application 1. Reschedule the job to it's
			 * original schedule. 2. Resume the job to keep the latest schedule. It will
			 * also restart the job that been paused. 3. Do nothing, the jobs will remain
			 * the same as they are before the restart.
			 * 
			 * Here we choose option 2. The benefit is in the future we can use the
			 * reschedule function to manage the schedule in DB. We can rely on the
			 * persistent of the schedule in DB.
			 */
