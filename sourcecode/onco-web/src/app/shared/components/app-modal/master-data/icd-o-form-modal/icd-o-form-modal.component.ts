...
@Component({
    selector: "app-icd-o-form-modal",
    templateUrl: "./icd-o-form-modal.component.html",
    styleUrls: ["./icd-o-form-modal.component.scss"]
})
export class IcdOFormModalComponent implements OnInit, AfterViewInit {

    public icdOForm: FormGroup;
    ...
    ngAfterViewInit(): void {
        if (this.selected) {
            ...
            this.icdOForm.patchValue(values);
            this.cd.detectChanges();
        }
    }

    private init(): void {
        console.log("I:--START--:--OnLoad IcdOFormModalComponent--");
    ...
    private clearInputError(inputName: string): void {
        this.util.clearInputError(this.icdOForm, inputName);
    }

    private addInputError(inputName: string, errors: any): void {
        this.util.addInputError(this.icdOForm, inputName, errors);
    }

    public isDateInvalid(inputName: string) {
        this.clearInputError(inputName);
        const dt = this.icdOForm.value[inputName];
    ...
    public checkDuplicateVersion(): void {
        ...
        if (!this.util.isNullOrEmpty(this.icdOForm.value.version)) {
            const req: any = {};
            req.type = 'ICD-O';
            req.id = this.icdOForm.value.dataVerId;
            req.version = this.icdOForm.value.version;
    ...
    public onSave(): void {
        ...
        if (this.icdOForm.valid && this.validateFile()) {
            ...
            const serviceName = this.constants.SERVICE_NAME.UPLOAD_ICD_O;
    ...
    private get formData(): FormData {
        ...
        input.append('importFile', this.icdOForm.value.importFile);
        return input;
    }
    ...
    public handleFileSelect(files: any): void {
        ...
        if (isFileUploadValid) {
            this.icdOForm.get('importFile').setValue(this.file);
        }
    }

    private get req() {
        const dataVersion: any = {};
        dataVersion.id = this.icdOForm.value.dataVerId;
        dataVersion.version = this.icdOForm.value.version;
        dataVersion.dataDate = this.icdOForm.value.dataDate;
        dataVersion.isDefault = this.icdOForm.value.markAsDefault ? this.constants.YES : this.constants.NO;
        dataVersion.type = 'ICD-O';
    ...
    public deleteAttachment() {
        ...
        modalRef.subscribe((confirm: any) => {
            if (confirm) {
                this.file = undefined;
                this.icdOForm.get('importFile').setValue(null);
            }
        });
    }
    ...
    public get isEdit(): boolean {
        return !this.util.isNullOrEmpty(this.icdOForm.value.dataVerId);
    }
}
