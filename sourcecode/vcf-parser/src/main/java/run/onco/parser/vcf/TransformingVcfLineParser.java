...
/**
 * Applies a transformation to a VCF file. This is a streaming
 * {@link VcfLineParser} that applies a {@link VcfTransformation} and writes the
 * VCF. More generally, can apply to a single VCF stream a set of
 * transformations and corresponding writers.
 * <p>
 * Example:
 * 
 * <pre>
...
public class TransformingVcfLineParser implements VcfLineParser, Closeable {
	...
	private TransformingVcfLineParser(@Nonnull List<VcfTransformation> transformations,
			@Nonnull List<VcfWriter> writer) {
	...
	public void close() throws IOException {
		m_writers.forEach(run.onco.parser.vcf.VcfWriter::close);
	}

	public static class Builder {

		private List<VcfTransformation> m_transformations = new ArrayList<VcfTransformation>();
		private List<VcfWriter> m_writers = new ArrayList<VcfWriter>();
