...
/**
 * A simple {@link org.pharmgkb.parser.vcf.VcfLineParser} that loads an entire
 * VCF file into memory to permit constant-time lookup for VCF records, either:
 * <ul>
 * <li>By any ID listed in the {@code ID} field, or</li>
 * <li>By chromosome and position</li>
 * </ul>
 * <p>
 * <em>This implementation is memory-intensive and should only be used for short
 * VCF files where repeated arbitrary (random) access to VCF records is
 * required.</em>
 * <p>
 * By default, an {@link java.lang.IllegalArgumentException} is thrown each time
 * a duplicate ID or locus is found. To change this behavior, see
 * {@link Builder#setDuplicateLocusHandler} and
 * {@link Builder#setDuplicateLocusHandler}.
 *
...
public class MemoryMappedVcfLineParser implements VcfLineParser {
	...
	public void parseLine(VcfMetadata metadata, VcfPosition position, List<VcfSample> sampleData) {
		...
		// link by locus
		MemoryMappedVcfDataStore.Locus locus = new MemoryMappedVcfDataStore.Locus(position.getChromosome(),
	...
	public static class Builder {
		...
		/**
		 * Determines what to do when an ID that was previously set is encountered,
		 * regardless of whether the two IDs correspond to the same locus.
		 *
		 * This is independent of {@link #setDuplicateLocusHandler(DuplicateHandler)},
		 * except when either is set to {@link DuplicateHandler#FAIL}.
		 */
		...
		/**
		 * Determines what to do when a VCF record is encountered with a locus
		 * (chromosome and position) that was already found.
		 *
	...
	/**
	 * What to do when a duplicate VCF record (line) is encountered. This includes
	 * cases where the contents of the record, such as the ALT, REF, INFO, and
	 * sample fields, differ.
	 */
	public enum DuplicateHandler {

		/**
		 * Return an {@link java.lang.IllegalArgumentException} when a duplicate is
		 * encountered.
		 */
		FAIL,

		/**
		 * Always keep the record that was previously read. That is, don't process
		 * duplicate records.
		 */
		KEEP_FIRST,

		/**
		 * Replace the {@link org.pharmgkb.parser.vcf.model.VcfSample VcfSamples} and
		 * {@link org.pharmgkb.parser.vcf.model.VcfPosition}. In other words, ignore the
		 * fact that the record is a duplicate and record it anyway.
		 */
		KEEP_LAST
	}
