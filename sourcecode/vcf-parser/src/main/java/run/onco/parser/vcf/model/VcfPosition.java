...
public class VcfPosition {
  ...
  private final static Logger logger = Logger.getLogger(VcfPosition.class);
      ...
      for (String f : filter) {
        ...
        if (f.equals("PASS")) {
          if (filter.size() == 1) { // a user is likely to pass "PASS" instead of an empty list or null
            logger.warn("FILTER is PASS, but should have been passed as null. Converting to null");
            filter = null;
